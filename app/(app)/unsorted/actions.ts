...
export async function saveFileAsTransactionAction(
  ...
  try {
    ...
    // Create transaction (with dedup check unless forceSave)
    const forceSave = formData.get("forceSave") === "true"
    const result = await createTransaction(user.id, validatedForm.data as TransactionData, forceSave)
    ...
    if (result.status === "duplicate_found") {
      return {
        ...
        duplicateData: {
          ...
          newTransactionData: result.newTransactionData as Record<string, unknown>,
          resumeIndex: 0,
        },
      }
    }
