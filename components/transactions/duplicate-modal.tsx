...
export function DuplicateModal({
  ...
  const newData = duplicateData.newTransactionData as Record<string, unknown>
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-base">
            We found an existing transaction that matches the date, amount, and merchant of this new entry. What would you like to do?
          </DialogDescription>
        </DialogHeader>
          ...
          <Card className="border-blue-200 bg-blue-50/30 dark:border-blue-900/50 dark:bg-blue-900/10">
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">Date</span>
                <span className="font-medium">
                  {newData.issuedAt ? format(new Date(newData.issuedAt as string), "MMM dd, yyyy") : "N/A"}
                </span>
              </div>
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">Merchant</span>
                <span className="font-medium truncate ml-4 text-right">{(newData.merchant as string) || "Unknown"}</span>
              </div>
              <div className="flex justify-between border-b pb-1">
                <span className="text-muted-foreground">Amount</span>
                <span className="font-bold text-base">
